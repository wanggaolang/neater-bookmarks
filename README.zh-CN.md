...
| 输入 | 效果 |
|---|---|
| `github` | 综合搜索——书签名和 URL 中均会匹配（默认） |
| `t:github` | 仅搜索**书签名**，搜索框下方出现红色「标题模式」提示 |
| `u:github.com` | 仅搜索 **URL**，搜索框下方出现红色「URL 模式」提示 |

空格表示 **AND**——所有词项必须同时命中。例如 `t:ab bc` 只返回书签名中同时含 "ab" 和 "bc" 的结果。前缀大小写不敏感（`T:`、`U:` 均有效），冒号后允许有空格（`t: github`）。
